...
def corrupt_signal(input_signal, noise_type, sigma = 1.0,
    ...
    if noise_type == 'awgn':
        ...
        fading_h = np.sqrt(np.random.standard_normal(data_shape)**2 +  np.random.standard_normal(data_shape)**2)/np.sqrt(2.0)
        noise = sigma * np.random.standard_normal(data_shape) # Define noise

        # corrupted_signal = 2.0*fading_h*input_signal-1.0 + noise
        corrupted_signal = 2.0*input_signal-1.0 + noise/fading_h
